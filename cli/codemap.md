...
## Integration Points
- `github.com/ap0ught/tmuxai/config`: reads effective config and environment overrides via `config.Load(...)`.
- `github.com/ap0ught/tmuxai/internal`: constructs and drives the runtime `Manager` and calls high-level methods for KB loading, model switching, and session execution.
- `github.com/ap0ught/tmuxai/logger`: structured runtime logging for config/mode/bootstrap and error reporting.
